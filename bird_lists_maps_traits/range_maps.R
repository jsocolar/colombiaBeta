...
# are planar. I think the above is equivalent to checking for intersections on a 
...
t2 <- rbind(t2, data.frame(orig_sort = NA, concept_8 = NA, latin_name = NA, 
                           TAXON_nid_MATCHED = NA, HBW_CAT = "sp",
                           HBW_LATIN = c("Catharus maculatus", "Pyrrhura chapmani"), 
                           HBW_LATIN_rev = NA, HBW_EN_EBIRD = NA,
                           HBW.Clem = "sp-ssp", cat_match_2019 = "HBW split", 
                           sci_match = NA, CLEM_SORT = NA,
                           CLEM_SCI_2019 = c("Catharus dryas", "Pyrrhura melanura"), 
                           CLEM_ENG_2019 = NA, notes = NA, CLEM_CAT_2019 = NA, 
                           SPECIES_CODE_2019 = NA, CLEM_SPECIES_CODE_2018 = NA, 
                           range = NA, SORT_INTEGRATED = NA))

csp2 <- t2[t2$HBW_LATIN %in% colombia_species, ]

elevation_dataset <- read.csv("/Users/jacobsocolar/Dropbox/Work/Colombia/Data/Birds/Bird_elevations_modified.csv", 
                              stringsAsFactors = F)
...
# elevation dataset. Many of these do not occur in Colombia. Those that do are 
# mostly splits or naming issues.
...
##### Hydrosheds #####
# NO OUTPUTS- suggest removal
#This code is nothing at the moment--just a place to save some bits that will 
# potentially be useful to have in the future
# 
