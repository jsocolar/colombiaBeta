# Comparing "ragged" model formulation, with partial_sum version, to check 
# partial_sum function works as expected before getting wrapped in reduce_sum
#
# Simulating under:
...
#
# I'm satisfied that they are producing the same result
...
stan_data <- list(
    ...
    # entries for covariates. You could reduce the memory requirement further
    # by passing it a much smaller vector which is then indexed by species id
    # it's a bit non-intuitive in stan formatted this way
...
# Run partial sum version ----
mod_pS <- cmdstan_model("stan_files/occupancyMod_ragged_parallel_pS.stan")
time_pS <- system.time(samps_pS <- mod_pS$sample(data = stan_data,
                                             num_chains = num_chains,
                                             num_cores = num_chains))
...
sub_pS <- samps_pS$draws() %>%
    subset_draws(., variable = c("mu_b0", "sigma_b0", "mu_d0", "sigma_d0"))
sub_both <- bind_draws(sub_pS, sub_pS, along="chain")

# Reduce filesize and save workspace
rm(list = c("samps_pS", "samps_R"))
save.image("workspace/compare_ragged_partialSum.RData")
