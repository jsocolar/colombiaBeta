...
functions{
  real partial_sum(
        ...
        // Occupancy
          ...
          // Trait effects; covariates vary by species and all slopes are universal
            ...
            // Functional traits
              real b3_migratory,
              real b3_mass,
              // diet (one-hot; reference category = omnivore)
                real b3_dietInvert,
                real b3_dietCarn,
          ...
          // pasture-x-trait interactions: covariates vary by species and all slopes are universal. Identical to a subset of b3 terms, but multipled by the pasture covariate.
            // Biogeography
    ...
    // variables for computation
      vector[len] lp;             // a vector to store the log-probability contribution from each row of the data slice
      vector[len] logit_psi;             // logit_psi at every row of the data slice
      row_vector[4] logit_theta;  // logit_theta for the visits at one individual species-point. 
                      // For points with < 4 visits, the trailing elements of logit_theta will be nonsense, computed from trailing 0s on the visit covariates.
                      // det_data has trailing -1s, thus ensuring that none of these nonexistent visits accidentally slips into analysis.
      vector[len] logit_theta_vector; // vectorized computation of the part of logit_theta that can be represented across points as a vector (i.e. no visit-specific covatiates)
      matrix[len, 4] logit_theta_matrix;  // vectorized computation of the part of logit_theta that includes visit-specific covariates
    // Computation:
      logit_psi = 
        b0_spCl[id_spCl[start:end]] + 
        b0_sp[id_sp[start:end]] + b0_fam[id_fam[start:end]] +
        
        b1_relev_sp[id_sp[start:end]].*relev[start:end] + b1_relev2_sp[id_sp[start:end]].*relev2[start:end] +
        b1_lowland*lowland[start:end] + b1_x_lowland_relev*lowland_x_relev[start:end] + b1_x_lowland_relev2*lowland_x_relev2[start:end] +

        b2_pasture_sp[id_sp[start:end]].*pasture[start:end] + b2_pasture_fam[id_fam[start:end]].*pasture[start:end] +
          
        b3_mountain_barrier*mountain_barrier[start:end] + b3_valley_barrier*valley_barrier[start:end] +
        b3_eastOnly*eastOnly[start:end] + b3_westOnly*westOnly[start:end] + b3_snsmOnly*snsmOnly[start:end] + b3_notEandes*notEandes[start:end] + b3_notWandes*notWandes[start:end] +
        b3_elevMedian*elevMedian[start:end] + b3_elevBreadth*elevBreadth[start:end] + 
        b3_forestPresent*forestPresent[start:end] + b3_forestSpecialist*forestSpecialist[start:end] + b3_tfSpecialist*tfSpecialist[start:end] + 
        b3_dryForestPresent*dryForestPresent[start:end] + b3_floodDrySpecialist*floodDrySpecialist[start:end] + b3_floodSpecialist*floodSpecialist[start:end] +
        b3_aridPresent*aridPresent[start:end] +
        b3_migratory*migratory[start:end] + b3_mass*mass[start:end] + 
        b3_dietInvert*dietInvert[start:end] + b3_dietCarn*dietCarn[start:end] + b3_dietFruitNect*dietFruitNect[start:end] + b3_dietGran*dietGran[start:end] +
        b3_x_elevMedian_forestPresent*elevMedian_x_forestPresent[start:end] + b3_x_elevMedian_forestSpecialist*elevMedian_x_forestSpecialist[start:end] +
        ...
        b4_forestPresent*forestPresent_x_pasture[start:end] + b4_forestSpecialist*b3_forestSpecialist_x_pasture[start:end] + 
        b4_tfSpecialist*tfSpecialist_x_pasture[start:end] + b4_dryForestPresent*dryForestPresent_x_pasture[start:end] + 
        b4_floodDrySpecialist*floodDrySpecialist_x_pasture[start:end] + b4_floodSpecialist*floodSpecialist_x_pasture[start:end] +
        b4_aridPresent*aridPresent_x_pasture[start:end] +
        b4_migratory*migratory_x_pasture[start:end] + b4_mass*mass_x_pasture[start:end] + 
        b4_dietInvert*dietInvert_x_pasture[start:end] + b4_dietCarn*dietCarn_x_pasture[start:end] + b4_dietFruitNect*dietFruitNect_x_pasture[start:end] + 
        b4_dietGran*dietGran_x_pasture[start:end] +
        ...
        b5_distance_to_range_sp[id_sp[start:end]].*distance_to_range[start:end];
      
      logit_theta_vector = 
        d0_sp[id_sp[start:end]] + d0_fam[id_fam[start:end]] + 
        d1_pasture_sp[id_sp[start:end]].*pasture[start:end] + d1_pasture_fam[id_fam[start:end]].*pasture[start:end] +
        d3_mass*mass[start:end] + d3_elevMedian*elevMedian[start:end] + d3_migratory*migratory[start:end] + d3_dietCarn*dietCarn[start:end];
        
      logit_theta_matrix =
        diag_pre_multiply(d2_time_sp[id_sp[start:end]], time[start:end,]) + 
      ...
      for (r in 1:len) {  // loop over species-points
        logit_theta = logit_theta_vector[r] + logit_theta_matrix[r,];
        // likelihood
        if (Q_slice[r] == 1) {
          lp[r] = log_inv_logit(logit_psi[r]) +  // likelihood of occupancy
          bernoulli_logit_lpmf(det_slice[r, 1:nv_slice[r]] | logit_theta[1:nv_slice[r]]);   // likelihood of observed detection history given occupancy
        } else {
          lp[r] = log_sum_exp(
            log_inv_logit(logit_psi[r]) + // likelihood of occupancy
            sum(log1m_inv_logit(logit_theta[1:nv_slice[r]])), // likelihood of all-zero detection history given occupancy
...
transformed data{
  // pre-computed interactions
    ...
    vector[n_tot] b3_forestSpecialist_x_pasture = forestSpecialist .* pasture;
    ...
    vector[n_tot] floodSpecialist_x_pasture = floodSpecialist .* pasture;
    ...
    target += reduce_sum(
      ...
      // Data
        ...
        // pre-computed interactions
          ...
          b3_forestSpecialist_x_pasture, tfSpecialist_x_pasture, dryForestPresent_x_pasture, floodDrySpecialist_x_pasture,
          floodSpecialist_x_pasture, aridPresent_x_pasture, migratory_x_pasture, mass_x_pasture, dietInvert_x_pasture, 
          dietCarn_x_pasture, dietFruitNect_x_pasture, dietGran_x_pasture, elevMedian_x_forestPresent_x_pasture, 
          elevMedian_x_forestSpecialist_x_pasture, time_x_elev
    ); // end reduce_sum call
} // end model block
