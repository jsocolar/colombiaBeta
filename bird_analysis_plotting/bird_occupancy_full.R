...
saveRDS(samps_R_2, "/Users/jacobsocolar/Dropbox/Work/Colombia/Data/Analysis/Stan_outputs/samps_R_2.RDS")
...
stan_data_2 <- list(
  ...
  b2_pasture_sp_mult = fit1_summary$mad[grep("b1_pasture_sp\\[", fit1_summary$variable)],
...
mod_R <- cmdstan_model("/Users/jacobsocolar/Dropbox/Work/Code/colombiaBeta/stan_files/full_colombia_model/occupancyMod_ragged_parallel_v2.stan",
                       cpp_options = list(stan_threads = TRUE))
offset_test_samples <- mod_R$sample(data = stan_data_2, 
                          chains = 1,
                          threads_per_chain = 1,
                          refresh = 1,
                          iter_sampling = 500,
