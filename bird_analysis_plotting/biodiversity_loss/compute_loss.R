...
get_avg_cell_logratios <- function(forest_probs, pasture_probs, cutoff_type, cutoff){
  ...
  fp <- forest_probs[, 4:ncol(forest_probs)]
  pp <- pasture_probs[, 4:ncol(pasture_probs)]
  fp_pp <- cbind(fp, pp)
  ...
  aln <- apply(fp_pp, 1, avg_logratio)
...
get_regional_logratios <- function(forest_probs, pasture_probs, cutoff_type, cutoff, cell_positions = NULL){
  ...
  return(list(avg_logratio=avg_logratio, med_logratio=med_logratio))
}