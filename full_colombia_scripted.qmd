...
-   xy_lookup - TODO: remove this dependency.
...
Generate raster of distances and elevations across Colombia for predicting over. First need to rasterize the input layers (distance to range and elevation) to a 2x2 km grid. Note that the mask generated in the above script used at this stage.

```{r}
source("code/GIS_processing/raster_layer_prep.R")
```
...
With this in hand, then generate the data objects required for prediction
...
inputs from raster layer prep

Outputs:

-   outputs/prediction_info_dt.rds

-   outputs/xy_info_lookup.rds
...
-   outputs_old/unit_covariates.RDS -- TODO: can't find the place where unit_covariates.RDS is generated. Assuming it comes from flocker_model.R- need to confirm these are the same and place an explicit saveRDS call there.
